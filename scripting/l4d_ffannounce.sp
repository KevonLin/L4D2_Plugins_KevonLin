		...
		if (DamageCache[attackerc][i] != 0 && attackerc != i)
		{
			if (IsClientInGame(i) && IsClientConnected(i))
			{
				...
				switch(GetConVarInt(AnnounceType))
				{
					...
					{
						if (IsClientInGame(attackerc) && IsClientConnected(attackerc) && !IsFakeClient(attackerc))
							CPrintToChat(attackerc, "[{olive}FF{default}] {blue}You {default}did {green}%d {default}friendly fire damage to {blue}%s{default}.",DamageCache[attackerc][i],victim);
						if (IsClientInGame(i) && IsClientConnected(i) && !IsFakeClient(i))
							CPrintToChat(i, "[{olive}FF{default}] {blue}%s {default}did {green}%d {default}friendly fire damage to {blue}you{default}.",attacker,DamageCache[attackerc][i]);
					}
					case 2:
					{
						if (IsClientInGame(attackerc) && IsClientConnected(attackerc) && !IsFakeClient(attackerc))
