...
{	
	...
	{
		CPrintToChatAll("{default}[{olive}Vote{default}] Map restart in {green}%d{default} seconds.", READY_RESTART_MAP_DELAY+1);
		RestartMapDelayed();
	}
...
{ 
	...
	{	
		...
		{
			DrawPanelItem(menu, "回血(Disable)");
		}
		else
		{
			DrawPanelItem(menu, "回血");
		}
		if (VotensAlltalkE_D == false)
		{ 
			DrawPanelItem(menu, "开启全体语音(Disable)");
		}
		else
		{
		...
		{
			DrawPanelItem(menu, "关闭全体语音(Disable)");
		}
		else
		{
		...
		{
			DrawPanelItem(menu, "重置当前地图(Disable)");
		}
		else
		{
		...
		{
			DrawPanelItem(menu, "更换地图(Disable)");
		}
		else
		{
			DrawPanelItem(menu, "更换地图");
		}

		if (VotensMap2E_D == false)
		{
			DrawPanelItem(menu, "更换第三方图(Disable)");
		}
		else
		{
			DrawPanelItem(menu, "更换第三方图");
		}

		if (g_bVotensKickED == false)
		{
			DrawPanelItem(menu, "踢出玩家(Disable)");
		}
		else
		{
		...
		{
			DrawPanelItem(menu, "强制玩家旁观(Disable)");
		}
		else
		{
	...
	{
		CPrintToChat(client, "{default}[{olive}Vote{default}] 投票插件已关闭!");
	}
		...
		switch (itemNum)
		{
			case 1: 
			{
				if (VotensHpE_D == false)
				{
					...
					CPrintToChat(client, "{default}[{olive}Vote{default}] 禁用回血");
				}
			...
			case 2: 
			{
				...
				{
					FakeClientCommand(client,"sm_votes");
					CPrintToChat(client, "{default}[{olive}Vote{default}] 禁用全体语音");
				}
			...
			case 3: 
			{
				...
				{
					FakeClientCommand(client,"sm_votes");
					CPrintToChat(client, "{default}[{olive}Vote{default}] 禁用关闭全体语音");
				}
			...
			case 4: 
			{
				...
				{
					FakeClientCommand(client,"sm_votes");
					CPrintToChat(client, "{default}[{olive}Vote{default}] 禁用重启当前地图");
				}
			...
			case 5: 
			{
				...
				{
					FakeClientCommand(client,"sm_votes");
					CPrintToChat(client, "{default}[{olive}Vote{default}] 禁用更换地图");
				}
			...
			case 6: 
			{
				...
				{
					FakeClientCommand(client,"sm_votes");
					CPrintToChat(client, "{default}[{olive}Vote{default}] 禁用更换第三方地图");
				}
			...
			case 7: 
			{
				...
				{
					FakeClientCommand(client,"sm_votes");
					CPrintToChat(client, "{default}[{olive}Vote{default}] 禁用踢人");
				}
			...
			case 8: 
			{
				...
				{
					FakeClientCommand(client,"sm_votes");
					CPrintToChat(client, "{default}[{olive}Vote{default}] 禁用强制旁观玩家");
				}
...
public Action Command_VoteHp(int client, int args)
{
	...
	&& VotensHpE_D == true)
	{
		...
		{
			CPrintToChatAll("{default}[{olive}Vote{default}]{olive} %N {default}starts a vote: {blue}give hp",client);
			...
			SetMenuTitle(g_hVoteMenu, "恢复全部幸存者生命值?");
	...
	{
		CPrintToChat(client, "{default}[{olive}Vote{default}] This vote is prohibited");
	}
	return Plugin_Handled;
}
public Action Command_VoteAlltalk(int client, int args)
{
	...
	&& VotensAlltalkE_D == true)
	{
		...
		if(CanStartVotes(client))
		{
			CPrintToChatAll("{default}[{olive}Vote{default}]{olive} %N {default}starts a vote: {blue}turn on alltalk",client);
			...
			g_hVoteMenu = CreateMenu(Handler_VoteCallback, MENU_ACTIONS_ALL);
			SetMenuTitle(g_hVoteMenu, "开启全体语音?");
			AddMenuItem(g_hVoteMenu, VOTE_YES, "Yes");
			AddMenuItem(g_hVoteMenu, VOTE_NO, "No");
...
public Action Command_VoteAlltalk2(int client, int args)
{
	...
	&& VotensAlltalk2E_D == true )
	{
		...
		if(CanStartVotes(client))
		{
			CPrintToChatAll("{default}[{olive}Vote{default}]{olive} %N {default}starts a vote: {blue}turn off alltalk",client);
			...
			g_hVoteMenu = CreateMenu(Handler_VoteCallback, MENU_ACTIONS_ALL);
			SetMenuTitle(g_hVoteMenu, "关闭全体语音?");
			AddMenuItem(g_hVoteMenu, VOTE_YES, "Yes");
			AddMenuItem(g_hVoteMenu, VOTE_NO, "No");
...
public Action Command_VoteRestartmap(int client, int args)
{
	...
	&& VotensRestartmapE_D == true)
	{
		...
		if(CanStartVotes(client))
		{
			CPrintToChatAll("{default}[{olive}Vote{default}]{olive} %N {default}starts a vote: {blue}restartmap",client);
			...
			g_hVoteMenu = CreateMenu(Handler_VoteCallback, MENU_ACTIONS_ALL);
			SetMenuTitle(g_hVoteMenu, "重新开始当前地图?");
			AddMenuItem(g_hVoteMenu, VOTE_YES, "Yes");
			AddMenuItem(g_hVoteMenu, VOTE_NO, "No");
...
public Action Command_VotesKick(int client, int args)
{
	...
	{
		CPrintToChat(client, "{default}[{olive}Vote{default}] Kick Player is prohibited");
	}	
	return Plugin_Handled;
}
...
public int Menu_VotesKick(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		if(player && IsClientInGame(player))
		{
			...
			{
				CPrintToChat(param1, "{default}[{olive}Vote{default}] Target has kick immue, choose again!");
				CPrintToChat(player, "{default}[{olive}Vote{default}] {olive}%N{default} tries to kick you, but you have kick immue.", param1);
		...
		{
			CPrintToChatAll("{default}[{olive}Vote{default}] Target is not in game, choose again!");
			CreateVoteKickMenu(param1);
		}	
	}
	else if ( action == MenuAction_Cancel)
...
public void DisplayVoteKickMenu(int client)
{
	...
	if(CanStartVotes(client))
	{
		...
		CPrintToChatAll("{default}[{olive}Vote{default}]{olive} %N {default}starts a votes: {blue}kick %s", client, swapplayername);
		...
		g_hVoteMenu = CreateMenu(Handler_VoteCallback, MENU_ACTIONS_ALL); 
		SetMenuTitle(g_hVoteMenu, "踢掉玩家 %s ?",swapplayername);
		AddMenuItem(g_hVoteMenu, VOTE_YES, "Yes");
		AddMenuItem(g_hVoteMenu, VOTE_NO, "No");
		SetMenuExitButton(g_hVoteMenu, false);
		VoteMenuToAll(g_hVoteMenu, 20);
...
public Action Command_VotemapsMenu(int client, int args)
{
	if(g_bEnable == true && VotensMapE_D == true)
	{
		...
		SetMenuTitle(menu, "选择地图");
		if(game_l4d2)
		{
			AddMenuItem(menu, "c1m1_hotel", "C1 Dead Center (4 Maps)");
			AddMenuItem(menu, "c2m1_highway", "C2 Dark Carnival (5 Maps)");
			AddMenuItem(menu, "c3m1_plankcountry", "C3 Swamp fever (4 maps)");
			AddMenuItem(menu, "c4m1_milltown_a", "C4 Hard Rain (5 Maps)");
			AddMenuItem(menu, "c5m1_waterfront", "C5 The Parish (5 Maps)");
			AddMenuItem(menu, "c6m1_riverbank", "C6 The Passing (3 Maps)");
			AddMenuItem(menu, "c7m1_docks", "C7 The Sacrifice (3 Maps)");
			AddMenuItem(menu, "c8m1_apartment", "C8 No Mercy (5 Maps)");
			AddMenuItem(menu, "c9m1_alleys", "C9 Crash Course (2 Maps)");
			AddMenuItem(menu, "c10m1_caves", "C10 Death Toll (5 Maps)");
			AddMenuItem(menu, "c11m1_greenhouse", "C11 Dead Air (5 Maps)");
			AddMenuItem(menu, "c12m1_hilltop", "C12 Blood Harvest (5 Maps)");
			AddMenuItem(menu, "c13m1_alpinecreek", "C13 Cold Stream (4 Maps)");
			AddMenuItem(menu, "c14m1_junkyard", "C14 The Last Stand (2 Maps)");
		}
		else
		{
	...
	{
		CPrintToChat(client, "{default}[{olive}Vote{default}] Change map vote is prohibited");
	}
	return Plugin_Handled;
}
...
public Action Command_Votemaps2Menu(int client, int args)
{
	...
	{
		CPrintToChat(client, "{default}[{olive}Vote{default}] Change Custom map vote is prohibited");
	}
	return Plugin_Handled;
}
...
public void DisplayVoteMapsMenu(int client)
{
	...
	if(CanStartVotes(client))
	{
		...
		CPrintToChatAll("{default}[{olive}Vote{default}]{olive} %N {default}starts a vote: {blue}change map %s", client, votesmapsname);
...
public Action Command_Votesforcespectate(int client, int args)
{
	...
	{
		CPrintToChat(client, "{default}[{olive}Vote{default}] Forcespectate Player is prohibited");
	}
	return Plugin_Handled;
}
...
public int Menu_Votesforcespectate(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		if(player && IsClientInGame(player))
		{
			if(HasAccess(player, g_sKickImmueAccesslvl))
			{
				CPrintToChat(param1, "{default}[{olive}Vote{default}] Target has forcespectate immunity, choose again!");
				CPrintToChat(player, "{default}[{olive}Vote{default}] {olive}%N{default} tries to forcespectate you, but you have kick immue.", param1);
...
public void DisplayVoteforcespectateMenu(int client)
{
	...
	if(CanStartVotes(client))
	{
		...
		CPrintToChatAll("{default}[{olive}Vote{default}]{olive} %N {default}starts a vote: {blue}forcespectate player %s{default}, only their team can vote", client, swapplayername);
		...
		g_hVoteMenu = CreateMenu(Handler_VoteCallback, MENU_ACTIONS_ALL); 
		SetMenuTitle(g_hVoteMenu, "强制旁观玩家 %s?",swapplayername);
		AddMenuItem(g_hVoteMenu, VOTE_YES, "Yes");
		AddMenuItem(g_hVoteMenu, VOTE_NO, "No");
		SetMenuExitButton(g_hVoteMenu, false);
...
public int Handler_VoteCallback(Menu menu, MenuAction action, int param1, int param2)
{
	...
	if(action == MenuAction_Select)
	{
		switch(param2)
		{
			...
			{
				Votey += 1;
				//CPrintToChatAll("[{olive}Vote{default}] %N {blue}has voted{default}.", param1);
			}
			case 1: 
			{
				Voten += 1;
	...
	else if ( action == MenuAction_Cancel)
	{
		...
		{
			//CPrintToChatAll("[{olive}Vote{default}] %N {blue}abandons the vote{default}.", param1);
		}
	}
	//==========================
	...
	{
		CPrintToChatAll("{default}[{olive}Vote{default}] No votes");
	...
	else if (action == MenuAction_VoteEnd)
	{
		if ((strcmp(item, VOTE_YES) == 0 && FloatCompare(percent, g_fLimit) < 0 && param1 == 0) || (strcmp(item, VOTE_NO) == 0 && param1 == 1))
		{
			...
			CreateTimer(1.0, Timer_VoteDelay, _, TIMER_REPEAT| TIMER_FLAG_NO_MAPCHANGE);
			// EmitSoundToAll("ui/beep_error01.wav");
			CPrintToChatAll("{default}[{olive}Vote{default}] {red}Vote fail.{default} At least {red}%d%%%%{default} to agree.(agree {green}%d%%%%{default}, total {green}%i {default}votes)", RoundToNearest(100.0*g_fLimit), RoundToNearest(100.0*percent), totalVotes);
			CreateTimer(2.0, VoteEndDelay);
		}
		else
		{
			...
			CPrintToChatAll("{default}[{olive}Vote{default}] {blue}Vote pass.{default}(agree {green}%d%%%%{default}, total {green}%i {default}votes)", RoundToNearest(100.0*percent), totalVotes);
			CreateTimer(2.0, VoteEndDelay);
...
public Action Timer_forcespectate(Handle timer, any client)
{
	...
	if (g_iSpectatePenaltyCounter[client] != 0)
	{
		if (GetClientTeam(client) == 3||GetClientTeam(client) == 2)
		{
			...
			CPrintToChat(client, "{default}[{olive}Vote{default}] You have been voted to be forcespectated! Wait {green}%ds {default}to rejoin team again.", g_iSpectatePenaltyCounter[client]);
	...
	else if (g_iSpectatePenaltyCounter[client] == 0)
	{
		...
		{
			CPrintToChat(client, "{default}[{olive}Vote{default}] You can rejoin both team now.");	//only print this hint text to the spectator if he tried to join the infected team, and got swapped before
		}
...
public void AnyHp()
{
	//CPrintToChatAll("{default}[{olive}Vote{default}] All players{blue}");
...
bool TestVoteDelay(int client)
{
 	...
 	if (delay > 0)
 	{
 		if (delay > 60)
 		{
 			CPrintToChat(client, "{default}[{olive}Vote{default}] You must wait for {red}%i {default}sec then start a int vote!", delay % 60);
 		}
 		else
 		{
 			CPrintToChat(client, "{default}[{olive}Vote{default}] You must wait for {red}%i {default}sec then start a int vote!", delay);
 		}
 		return false;
 	}
...
bool CanStartVotes(int client)
{
	...
	{
		CPrintToChat(client, "{default}[{olive}Vote{default}] A vote is already in progress!");
		return false;
	}
	int iNumPlayers;
	//list of players
	...
	{
		CPrintToChat(client, "{default}[{olive}Vote{default}] Vote cannot be started. Not enough {red}%d {default}players.", g_iCvarPlayerLimit);
		return false;
	}
	return true;
}
...
public Action COLD_DOWN(Handle timer,any client)
{
	switch (g_voteType)
	{
		...
		case (view_as<voteType>(map)):
		{
			...
			CPrintToChatAll("[{olive}Vote{default}] {green}5{default} sec to change map {blue}%s",votesmapsname);
		...
		{
			CPrintToChatAll("[{olive}Vote{default}] {blue}%s{default} has been kicked!", swapplayername);
		...
		case (view_as<voteType>(forcespectate)):
		{
			...
			{
				CPrintToChatAll("[{olive}Vote{default}] {blue}%s{default} has been forcespectated!", swapplayername);
			...
			{
				CPrintToChatAll("[{olive}Vote{default}] %s player not found", swapplayername);	
			}
		}
	}
